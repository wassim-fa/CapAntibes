import { IContentByLang } from '@/interfaces'

export const contentsBabaRestaurant: Record<string, IContentByLang> = {
  title: {
    fr: `restaurant bar plage`,
    en: `restaurant bar plage`,
    ru: `restaurant bar plage`
  },
  intro: {
    fr: `BaBa (Ba, בא , en hébreu signifie venir) est un lieu de partage et de mouvement. C’est une fête qui relie les rives de la Méditerranée. C’est un grand soleil qui monte à la tête. Baba est orchestré avec la complicité du chef As-saf Granit, né dans la ville cosmopolite de Jérusalem et qui est l’âme d’adresses iconiques à travers le monde, à l’ambiance informelle et élé-gante : le Balagan ou Tekes à Paris, Machneyuda à Jérusalem, le Coal Office à Londres, entre autres. Du restaurant en bord de sable et sa terrasse, au bar, à la plage ou à la piscine, vous êtes pris dans une même énergie qui anime l’hôtel. Celle d’une joie de vivre euphorisante.`,
    en: `BaBa (Ba, בא in Hebrew means "come") is a place of sharing and movement. It's a celebration that connects the shores of the Mediterranean Sea. It's a big sun that goes to your head. Baba is orchestrated with the complicity of chef Assaf Granit, born in the cosmopolitan city of Jerusalem, who is the soul of iconic addresses around the world, with an informal and elegant atmosphere: Balagan or Tekes in Paris, Machneyuda in Jerusalem, Coal Office in London, among others. From the restaurant on the sandy beach and its terrace, to the bar, the beach, or the pool, you are caught in the same energy that animates the hotel: the energy of a euphoric joy of living.`,
    ru: `Baba restaurant etoilé signa nordiuropon etris. Decteri ssendem audem iusquod condem ad monuline turebatiam vid consu inatodi ctorarit. Locciae invescia vero ve, quidendi pra, essenatabem consunum  et tunm effreviverum ex se ante factatque miliniq uostrum int. An vermis, fursul vid sedite confer audende ntiaede mquit. Untrenihicae moc. Muliam ium urnum poenduc idiis. Rorsupi cavero in tabit.`
  },
  toBookTable: {
    fr: `Réserver une table`,
    en: `Reserve a table`,
    ru: `réserver une table`
  },
  menu: {
    fr: `la carte`,
    en: `The card`,
    ru: `la carte`
  },
  chapter1: {
    fr: `DÉJEUNER`,
    en: `LUNCH`,
    ru: `DÉJEUNER`
  },
  subTitle1: {
    fr: `De 12h00 à 16h00`,
    en: `From 12:00 pm to 4:00 pm`,
    ru: `de 7h30 à 10h30`
  },
  text1: {
    fr: `Baba est un lieu sans pareil. Vous vous installez en bord de plage sur de grandes tablées d’amis et prolongez le bonheur et la fête tout au long de la journée. Baba propose une cuisine levantine, solaire et inédite. Les plats orientaux sont réinventés avec les produits locaux : grands mezze à partager et grillades au mangal (poulpes, viandes ou poissons) typiques d’Israël.  Les magnifiques saveurs du Moyen-Orient et l’histoire de la cuisine provençale s’accompagnent à table de cocktails et vins d’évasion, d’Israël, de Grèce, de Turquie.`,
    en: `Baba is a unique place. You settle down on large tables with friends by the beach and extend the happiness and celebration throughout the day. Baba offers a Levantine, solar and original cuisine. Eastern dishes are reinvented with local products: big mezze to share and grilled dishes on the mangal (octopuses, meat or fish) typical of Israel. The magnificent flavors of the Middle East and the history of Provencal cuisine are accompanied at the table by cocktails and wines from Israel, Greece, and Turkey.`,
    ru: `Le matin signa nordiuropon etris. Decteri ssendem audem iusquod condem ad mo turebatiam vid consu inatodi ctorarit. Locciae invescia vero ve, quidendi pra, essenatabem consunum effreviverum ex se ante factatque miliniq uostrum int. An vermis, fursul vid sedite confer audende ntiaede mquit. Untrenihicae moc. Muliam ium urnum poenduc idiis. Rorsupi cavero in tabit.`
  },
  chapter2: {
    fr: `Dîner`,
    en: `Dinner`,
    ru: `dîner`
  },
  subTitle2: {
    fr: `de 19h00 à 23h30`,
    en: `From 7:00 pm to 11:30 pm`,
    ru: `de 19h00 à 23h00`
  },
  text2: {
    fr: `Le matin signa nordiuropon etris. Decteri ssendem audem iusquod condem ad mo turebatiam vid consu inatodi ctorarit. Locciae invescia vero ve, quidendi pra, essenatabem consunum effreviverum ex se ante factatque miliniq uostrum int. An vermis, fursul vid sedite confer audende ntiaede mquit. Untrenihicae moc. Muliam ium urnum poenduc idiis. Rorsupi cavero in tabit.`,
    en: `In the coolness of the evening, the atmosphere calms down. The decor is imbued with elegance and sweetness. The colors are subdued, and by candlelight, everything regains its calm. You dine accompanied by the sound of the waves. Spices escape from the open-air kitchen and perfume the atmosphere. Around the bar, you prolong the dinner for a last drink.`,
    ru: `Le matin signa nordiuropon etris. Decteri ssendem audem iusquod condem ad mo turebatiam vid consu inatodi ctorarit. Locciae invescia vero ve, quidendi pra, essenatabem consunum effreviverum ex se ante factatque miliniq uostrum int. An vermis, fursul vid sedite confer audende ntiaede mquit. Untrenihicae moc. Muliam ium urnum poenduc idiis. Rorsupi cavero in tabit.`
  },
  chapter3: {
    fr: `Le bar`,
    en: `The bar`,
    ru: `Le bar`
  },
  subTitle3: {
    fr: `De 10h00 à 01h00`,
    en: `From 10:00 am à 1:00 am`,
    ru: `de 7h30 à 10h30`
  },
  text3: {
    fr: `Dès 10h00 et jusqu’à 1h00 du matin durant l’été, le bar du Beach Hôtel est un hymne à l’insouciance. La carte se compose de cocktails signatures aux parfums levantins. Le bar circulaire évoque les années folles de la Floride ou de Tel Aviv, comme un balcon sur l’horizon. A toute heure, au Bar Beach & Pool, une carte légère de finger food et de boissons est proposée. Autour de la piscine ou directement sur les bains de soleil de la plage, vous pouvez déguster salades, brochettes, frito misto…`,
    en: `From 10:00 am to 1:00 am during the summer, the Beach Hotel bar is an ode to carefreeness. The menu consists of signature cocktails with Levantine flavors. The circular bar evokes the wild years of Florida or Tel Aviv, like a balcony on the horizon. At any time, at the Beach & Pool Bar, a light menu of finger food and drinks is offered. Around the pool or directly on the sun loungers on the beach, you can enjoy salads, skewers, frito misto…`,
    ru: `Le matin signa nordiuropon etris. Decteri ssendem audem iusquod condem ad mo turebatiam vid consu inatodi ctorarit. Locciae invescia vero ve, quidendi pra, essenatabem consunum effreviverum ex se ante factatque miliniq uostrum int. An vermis, fursul vid sedite confer audende ntiaede mquit. Untrenihicae moc. Muliam ium urnum poenduc idiis. Rorsupi cavero in tabit.`
  },
  chapter4: {
    fr: `Baba glace`,
    en: `Baba glace`,
    ru: `Baba glace`
  },
  subTitle4: {
    fr: `De 14h00 à 18h00`,
    en: `From 2:00 pm to 6:00 pm`,
    ru: `de 7h30 à 10h30`
  },
  text4: {
    fr: `Ce sont les heures chaudes de Baba. Le chariot Baba Glaces s’installe sur la terrasse. Pour les plus jeunes et ceux qui souhaitent le rester : six parfums glacés, toujours renouvelés selon la saison et un sorbet oriental, comme la glace au tahini. Enfance servie en cornet ou en coupe.`,
    en: `These are the hot hours of Baba. The Baba Ice Cream cart is set up on the terrace. For the young and the young at heart: six ice cream flavors, always renewed according to the season, and an oriental sorbet, like tahini ice cream. Childhood served in a cone or in a cup.`,
    ru: `Le matin signa nordiuropon etris. Decteri ssendem audem iusquod condem ad mo turebatiam vid consu inatodi ctorarit. Locciae invescia vero ve, quidendi pra, essenatabem consunum effreviverum ex se ante factatque miliniq uostrum int. An vermis, fursul vid sedite confer audende ntiaede mquit. Untrenihicae moc. Muliam ium urnum poenduc idiis. Rorsupi cavero in tabit.`
  },
  chapter5: {
    fr: `BABA BEACH POOL`,
    en: `BABA BEACH POOL`,
    ru: `BABA BEACH POOL`
  },
  subTitle5: {
    fr: `de 7h30 à 10h30`,
    en: `de 7h30 à 10h30`,
    ru: `de 7h30 à 10h30`
  },
  text5: {
    fr: `Le matin signa nordiuropon etris. Decteri ssendem audem iusquod condem ad mo turebatiam vid consu inatodi ctorarit. Locciae invescia vero ve, quidendi pra, essenatabem consunum effreviverum ex se ante factatque miliniq uostrum int. An vermis, fursul vid sedite confer audende ntiaede mquit. Untrenihicae moc. Muliam ium urnum poenduc idiis. Rorsupi cavero in tabit.`,
    en: `Le matin signa nordiuropon etris. Decteri ssendem audem iusquod condem ad mo turebatiam vid consu inatodi ctorarit. Locciae invescia vero ve, quidendi pra, essenatabem consunum effreviverum ex se ante factatque miliniq uostrum int. An vermis, fursul vid sedite confer audende ntiaede mquit. Untrenihicae moc. Muliam ium urnum poenduc idiis. Rorsupi cavero in tabit.`,
    ru: `Le matin signa nordiuropon etris. Decteri ssendem audem iusquod condem ad mo turebatiam vid consu inatodi ctorarit. Locciae invescia vero ve, quidendi pra, essenatabem consunum effreviverum ex se ante factatque miliniq uostrum int. An vermis, fursul vid sedite confer audende ntiaede mquit. Untrenihicae moc. Muliam ium urnum poenduc idiis. Rorsupi cavero in tabit.`
  }
}
